...
export default function MembersPage() {
  ...
  const renderConnectButton = (member) => {
    ...
    return (
      <Button variant="outline" size="sm" className="w-full mt-4"
        onClick={(e) => handleConnect(e, member.id)} disabled={connectingIds.has(member.id)}>
        <UserPlus size={14} /> {connectingIds.has(member.id) ? 'Sending...' : 'Connect'}
      </Button>
    )
  }
            ...
            {filtered.map((member) => (
              <Link key={member.id} to={`/dashboard/members/${member.id}`} className="block no-underline">
                <Card hover className="text-center h-full">
                  <Avatar name={member.name} src={member.avatar_url} size="xl" className="mx-auto mb-4" />
